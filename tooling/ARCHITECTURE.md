...
| `@suluk/builder` | **the builder**: DSL; `buildApp`/`toShadcnRegistry`; **modules** (C021) — `installModule` (refuse-on-collision + `requires` + dangling-ref backstop + namespacing), curated + **remote** registries (`parseRegistry` untrusted, **`verifyRegistrySignature`** signed), `gradeModule`/`previewInstall`; **provider slots** (`swapProvider`) | ✅ 66 tests |
...
**Total: 20 TS packages (341 tests) + a Rust core (9 tests) = 350 green.** Shipped: npm (`@suluk/*`),
