...
| `@suluk/builder` | **the builder**: tiered contract-narrowing DSL; `buildApp` (backend+frontend) + `toShadcnRegistry`; **modules** (C021) — `installModule` (mergeable fragment, refuse-on-collision + `requires` + namespacing), a curated `FIRST_PARTY_REGISTRY` (ecommerce/crm/billing), `gradeModule` + `previewInstall` | ✅ 39 tests |
| `@suluk/deploy` | **the capstone**: swappable `DeployProvider`; Cloudflare (Workers + D1 + assets) | ✅ 8 tests |
| `@suluk/cockpit` | the **pure cockpit core** (cycle · builder · codegen · deploy · **drift** · **cross-cut** · validate/audit/preview) — shared brain | ✅ 62 tests |
...
**Total: 20 TS packages (311 tests) + a Rust core (9 tests) = 320 green.** Shipped: npm (`@suluk/*`),
